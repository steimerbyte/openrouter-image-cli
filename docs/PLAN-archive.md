# ARCHIVED — Plan: openrouter-image Rust CLI (pre-release planning notes)

> **Status: ARCHIVED.** This document captures the iterative planning that
> produced v0.1.0. The spec drifted several times during implementation;
> the final shipped behaviour is documented in [`README.md`](../README.md)
> and the per-release notes in [`CHANGELOG.md`](../CHANGELOG.md).
>
> Do not edit this file for current-scope changes — it is preserved for
> historical reference only.

## Original status (when this plan was written)
